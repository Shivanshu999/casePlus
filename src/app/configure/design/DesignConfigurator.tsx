...
interface DesignConfiguratorProps {
  configId: string;
  imageUrl: string;
  imageDimensions?: { width: number; height: number }; // Made optional
}
...
const DesignConfigurator = ({
  ...
  const dimensions = imageDimensions || defaultDimensions;
  ...
  const [options, setOptions] = useState<{
    ...
    phone: (typeof PHONE.options)[number];
    material: (typeof MATERIALS.options)[number];
    finish: (typeof FINISHES.options)[number];
  }>({
    color: COLORS[0],
    phone: PHONE.options[0],
                      ...
                      {PHONE.options.map((phone) => (
                        <DropdownMenuItem
                          ...
                          className={cn(
                            ...
                            {
                              "bg-zinc-100":
                                phone.label === options.phone.label,
                            }
                          )}
                          onClick={() => {
                            setOptions((prev) => ({ ...prev, phone }));
                          }}
                        >
                          <Check
                            className={cn(
